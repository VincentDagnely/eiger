...
for dc in $(seq 0 $((num_dcs - 1))); do
    for n in $(seq 0 $((nodes_per_dc - 1))); do
        ...
        mkdir -p ${src_dir}/cassandra_var/data/$global_node_num 2> /dev/null
        mkdir -p ${src_dir}/cassandra_var/commitlog/$global_node_num 2> /dev/null
        mkdir -p ${src_dir}/cassandra_var/saved_caches/$global_node_num 2> /dev/null
