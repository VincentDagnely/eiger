...
public class StressAction extends Thread
{
    ...
    public void run()
    {
        ...
        if (client.getOperation() == Stress.Operations.INSERT || client.getOperation() == Stress.Operations.COUNTER_ADD || client.getOperation() == Stress.Operations.INSERTCL || client.getOperation() == Stress.Operations.FACEBOOK_POPULATE || client.getOperation() == Stress.Operations.EXPLICIT_FACEBOOK_POPULATE)
    ...
    private Operation createOperation(int index)
    {
        ...
        case EXPLICIT_FACEBOOK_POPULATE:
            if (client.isCQL())
                throw new RuntimeException("CQL not support with this workload");
            return new ExplicitFacebookPopulator(client, index);

        case EXPLICIT_FACEBOOK:
            if (client.isCQL())
                throw new RuntimeException("CQL not support with this workload");
            return new ExplicitFacebookWorkload(client, index);
        }
